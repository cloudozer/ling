...
init([]) ->
	...
	{ok, [["minimal"]]} ->
	    ...
	    {ok, {SupFlags,
		  [File, Disk, Xenstore, Code, StdError, User, %%MK
	...
	_ ->
		...
		%%MK: 9p
		%%
		%% We retrieve the local mounting map needed for code loading by parsing
		%% a simple text file /boot/local.map. It is possible to use a standard
		%% application configuration facility but it is cumbersome. It pollutes
		%% the command line and load a few heavy modules for Erlang term parsing.
		%%
		LocalMap = read_local_map(),

		P9Server = {'9p_server',{'9p_server',start_link,[]},
					permanent,2000,worker,['9p','9p_server']},
		P9Mounter = {'9p_mounter',{'9p_mounter',start_link,[LocalMap,[sync]]},
					permanent,2000,worker,['9p','9p_mounter']},
...
%%MK
read_local_map() ->
	case erl_prim_loader:get_file("local.map") of
	{ok,MapBin,_} ->
		%% The local map is auto-generated, no format checks
		[{LocPath,AName}
				|| [LocPath,AName] <- [binary:split(Line, <<" ">>)
						||Line <- binary:split(MapBin, <<"\n">>, [global]),
								Line =/= <<>>]];
	error ->
		[]
	end.
...
%%MK
start_goofs() ->
    case init:get_argument(goofs) of
	{ok, [[MountAt|RawOpts]|_]} ->
		[{goofs,{goofs,start_link,[MountAt,RawOpts]},
				 permanent,2000,worker,[disk]}];
	_ ->
		[]
	end.
