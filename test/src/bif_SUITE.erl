...
display(Config) when is_list(Config) -> {skip,"start_node() not supported"}.
...
erl_bif_types(Config) when is_list(Config) -> {skip,"not documented"}.
...
guard_bifs_in_erl_bif_types(_Config) -> {skip,"not documented"}.
%%    ensure_erl_bif_types_compiled(),
%%
%%    List0 = erlang:system_info(snifs),
...
shadow_comments(_Config) -> {skip,"not documented"}.
%%    ensure_erl_bif_types_compiled(),
%%
%%    List0 = erlang:system_info(snifs),
...
binary_to_atom(Config) when is_list(Config) ->
    ...
    %%MK: ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	%%MK:      C <- lists:seq(16#10000, 16#8FFFF)],
    ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	      C <- lists:seq(16#10000, 16#100FF)],
    ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	      C <- lists:seq(16#8FF00, 16#8FFFF)],
    %%MK: ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	%%MK:      C <- lists:seq(16#90000, 16#10FFFF)],
    ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	      C <- lists:seq(16#90000, 16#900FF)],
    ?line [?BADARG(binary_to_atom(<<C/utf8>>, utf8)) ||
	      C <- lists:seq(16#10FF00, 16#10FFFF)],
...
erlang_halt(Config) when is_list(Config) ->
    ?line try erlang:halt(undefined) of
	_-> ?t:fail({erlang,halt,{undefined}})
    catch error:badarg -> ok end,
    ?line try halt(undefined) of
    ...
    ?line try halt(0, undefined) of
	_-> ?t:fail({halt,{0,undefined}})
    catch error:badarg -> ok end,
    ?line try halt(0, [undefined]) of
	_-> ?t:fail({halt,{0,[undefined]}})
    catch error:badarg -> ok end,
    ?line try halt(0, [{undefined,true}]) of
	_-> ?t:fail({halt,{0,[{undefined,true}]}})
    catch error:badarg -> ok end,
    ?line try halt(0, [{flush,undefined}]) of
	_-> ?t:fail({halt,{0,[{flush,undefined}]}})
    catch error:badarg -> ok end,
    ?line try halt(0, [{flush,true,undefined}]) of
