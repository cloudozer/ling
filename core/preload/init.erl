...
%%
%% LING-specific flags:
%%		  -nobeam		 : Code does not load .beam files
%%
...
boot(BootArgs) ->
	...
	%%
	%% When erlang:statistics(wall_clock) is called next time, the 'time since
	%% last call' will give the time elapsed since the beginning of the boot
	%% sequence.
	%%
...
boot(Start,Flags0,Args) ->
	...
	%%MK: inject default root
	Flags = case search('-root',Flags0) of
	{value,_} ->
		Flags0;
	_ ->
		[{'-root',[<<"/erlang">>]}|Flags0]
	...
	false ->
		spawn(fun() ->
			ensure_loaded(beam_lib),
			ensure_loaded(ling_bifs),
			ensure_loaded(ling_code),
			ensure_loaded(ling_disasm),
			ensure_loaded(ling_iops),
			ensure_loaded(ling_iopvars),
			ensure_loaded(ling_lib)
		end);
	_ ->
		ok %% do not load unnecessary modules for the fastest startup
	end,

    BootPid = do_boot(Flags,Start),
	
	%%MK: BootPid must receive 'start_em' to proceed
	if not IsNetworked ->
		BootPid ! start_em;
			true -> ok end,
...
setup_networking(Flags) ->
	case net_args(Flags) of
	{undefined,undefined,undefined,false} ->
		%% no external network
		false;
	{IpAddr,NetMask,Gateway,Dhcp} ->
		lwip:setup(IpAddr, NetMask, Gateway, Dhcp)
	end.

net_args(Flags) ->
	net_args(Flags, {undefined,undefined,undefined,false}).

net_args([], Conf) ->
	Conf;
net_args([{'-ipaddr',[IpAddr]}|Flags], {_,NetMask,Gateway,Dhcp}) ->
	net_args(Flags, {IpAddr,NetMask,Gateway,Dhcp});
net_args([{'-netmask',[NetMask]}|Flags], {IpAddr,_,Gateway,Dhcp}) ->
	net_args(Flags, {IpAddr,NetMask,Gateway,Dhcp});
net_args([{'-gateway',[Gateway]}|Flags], {IpAddr,NetMask,_,Dhcp}) ->
	net_args(Flags, {IpAddr,NetMask,Gateway,Dhcp});
net_args([{'-dhcp'}|Flags], {IpAddr,NetMask,Gateway,_}) ->
	net_args(Flags, {IpAddr,NetMask,Gateway,true});
net_args([_|Flags], Conf) ->
	net_args(Flags, Conf).
...
set_secrets(Secrets) ->
	case do_set_secrets(Secrets) of
	false ->
		exit({'cannot set shared secrets',Secrets});
	true ->
		ok
	end.
...
do_set_secrets([]) ->
	true;
do_set_secrets([Secret1,Secret2]) ->
	auth:set_secrets(Secret1, Secret2);	
do_set_secrets(_) ->
	false.

mount_flags(Flags) ->
	MntA = get_flag_args('-9p-',Flags,[]),
	MntZ = get_flag_args('-9p',Flags,[]),
	{MntA,MntZ}.

do_mounts({MntA,MntZ}) ->
	Specs = [{host_spec(Spec, [submerge]),pair_up(Ps)} || [Spec|Ps] <- MntA]
					++
			[{host_spec(Spec, []),pair_up(Ps)} || [Spec|Ps] <- MntZ],
	mount(Specs).

mount([]) ->
	ok;
mount([{{Host,Opts},Mounts}]) ->
	'9p_mounter':add_connection('9p_tcp', Host, Mounts, [sync|Opts]);
mount([{{Host,Opts},Mounts}|More]) ->
	'9p_mounter':add_connection('9p_tcp', Host, Mounts, Opts),
	mount(More).
...
pair_up([], Acc) ->
	reverse(Acc);
pair_up([A], _Acc) ->
	exit({'no attach point for',A});
pair_up([A,B|Ps], Acc) ->
	pair_up(Ps, [{B,A}|Acc]).	%%NB: reverse order

host_spec(Spec, Opts) ->
	case binary:match(Spec, <<":">>) of
	{S,_} ->
		HostB = binary:part(Spec, 0, S),
		N = size(Spec) -S -1,
		PortB = binary:part(Spec, size(Spec), -N),
		host_spec_1({binary_to_list(HostB),
					 list_to_integer(binary_to_list(PortB))}, Opts);
	nomatch ->
		host_spec_1({binary_to_list(Spec),564}, Opts)
	end.

host_spec_1({[$@|LingHost],Port}, Opts) ->
	{{LingHost,Port},Opts};
host_spec_1({DiodHost,Port}, Opts) ->
	{{DiodHost,Port},[{version,'9P2000.L'}|Opts]}.
